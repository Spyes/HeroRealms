...
let takeFromMarket = (~card: Card.card, ~market: Cards.cards, ~player: Player.player) => {
  switch (card.cost) {
    | Some(cost) =>
      switch (player.coins >= cost) {
        | true =>
          let player = {...player, discard: [card, ...player.discard], coins: player.coins - cost};
...
let make = (_children) => {
  {
      ...
      switch (action) {
        | ClickDeck => {
          switch (List.length(state.deck)) {
            ...
            | _ => {
              let ( newMarket, newDeck ) = drawFromDeck(~market=state.market, ~deck=state.deck);
              ReasonReact.Update({ ...state, market: newMarket, deck: newDeck })
            }
          }
        }
        | ClickMarketCard(card) => {
          let ( newPlayer, market ) = takeFromMarket(~card, ~market=state.market, ~player=state.players);
          let ( newMarket, newDeck ) = drawFromDeck(~market, ~deck=state.deck);
          ReasonReact.Update({ ...state, market: newMarket, players: newPlayer, deck: newDeck })
        }
    ...
    render: self => {
      ...
      <div className="Game">
        <Player player=players onClickHand={(~card, ~player) => self.send(ClickCardInHand(card, player))} />
        <Deck deck title="Deck" onClick={_event => self.send(ClickDeck)} />
        <Cards cards=market title="Market" onClick={(card) => self.send(ClickMarketCard(card))} />
      </div>
    }
  };
}
